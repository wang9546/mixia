...
Page({
  ...
  onPullDownRefresh() {
    Promise.all([this.loadTags(), this.loadList()]).then(() => {
  ...
  async loadTags() {
    try {
      const tagsRes = await db.getPhotoTagList({ pageSize: 100, status: 1 });
  ...
  async loadList() {
    ...
    try {
      const result = await db.getServiceList('wedding_photo', { pageSize: 100 });
      this.setData({
        list: result.list || [],
        loading: false
      });
  ...
  async onCoverChoose() {
    try {
      ...
      const cloudPath = `photos/${Date.now()}-${Math.random().toString(36).substr(2)}.jpg`;
  ...
  async onSubmit() {
    ...
    try {
      if (editingItem) {
        await db.updatePhoto(editingItem._id, formData);
      } else {
        await db.createPhoto(formData);
      }
