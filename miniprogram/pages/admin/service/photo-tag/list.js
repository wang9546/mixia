...
Page({
  ...
  async loadList() {
    ...
    try {
      const res = await db.getPhotoTagList({ pageSize: 100 });
      this.setData({
        list: res.list,
        loading: false
      });
  ...
  onDeleteTap(e) {
    ...
    wx.showModal({
      ...
      success: async (res) => {
        if (res.confirm) {
          try {
            await db.deletePhotoTag(item._id);
  ...
  async onSubmit() {
    ...
    try {
      if (editingItem) {
        await db.updatePhotoTag(editingItem._id, formData);
      } else {
        await db.createPhotoTag(formData);
      }
