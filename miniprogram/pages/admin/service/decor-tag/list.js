...
Page({
  ...
  async loadList() {
    ...
    try {
      const res = await db.getDecorTagList({ pageSize: 100 });
      this.setData({
        list: res.list,
        loading: false
      });
  ...
  onDeleteTap(e) {
    ...
    wx.showModal({
      ...
      success: async (res) => {
        if (res.confirm) {
          try {
            await db.deleteDecorTag(item._id);
  ...
  async onSubmit() {
    ...
    try {
      if (editingItem) {
        await db.updateDecorTag(editingItem._id, formData);
      } else {
        await db.createDecorTag(formData);
      }
