...
Page({
  ...
  async loadConfigs() {
    try {
      const { styles } = await db.getServiceConfigs('wedding_photo');
      this.setData({ tags: styles || [] });
    } catch (err) {
  ...
  onShareAppMessage() {
    if (this.data.info) {
      return {
        ...
        path: `/pages/dress/detail?id=${this.id}`,
  ...
  async loadDetail() {
    try {
      const info = await db.getServiceDetail('wedding_photo', this.id);
